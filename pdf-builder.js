// PDF BUILDER — Wohnen & WG-Regeln (B2.1 · Lecția 3)
(function () {
    ...
    function buildCast() {
        return `<div class="cast-banner">
            <h4>👋 Cast „Annettes Deutschkurs" — Andreea se mută într-o WG</h4>
            <div class="cast-grid">
                ...
                <div class="cast-card"><img src="images/andreea.png" alt="Andreea"><div class="name">Andreea 🇷🇴</div><div class="detail">Nouă în WG · Berlin</div></div>
                <div class="cast-card"><img src="images/mihai.png" alt="Mihai"><div class="name">Mihai</div><div class="detail">Coleg de WG · Berlin</div></div>
                <div class="cast-card"><img src="images/carolina.png" alt="Carolina"><div class="name">Carolina</div><div class="detail">Berlin</div></div>
                <div class="cast-card"><img src="images/florian.png" alt="Florian"><div class="name">Florian</div><div class="detail">Berlin</div></div>
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — Wohnen & WG-Regeln: Passiv (toate timpurile) + mit Modalverben + Passiversatzformen</h1>` + t;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillInBlock('Übung 1 — Passiv: Präsens & Präteritum', 'Präsens = wird + Partizip II · Präteritum = wurde + Partizip II (la final).', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillInBlock('Übung 2 — Passiv: Perfekt & Plusquamperfekt', 'ist/war + Partizip II + worden. ⚠️ La Passiv participiul lui werden e worden, NU geworden.', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillInBlock('Übung 3 — Passiv mit Modalverben', 'Modalverb (loc 2) + Partizip II + werden (la final). Ex: Die Tür muss abgeschlossen werden.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillInBlock('Übung 4 — Passiversatzformen ⭐', 'sein + zu + Infinitiv (ist abzuschließen) · sich lassen (lässt sich lösen) · man · -bar (trinkbar).', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += diktatBlock('Übung 5 — Diktat (Hörverstehen, vocea Claudiei)', 'Reguli de Hausordnung în Passiv — atenție la wird/muss … werden și la worden (NU geworden).', ex5Data);
        }
        return html;
    }
    ...
    function buildVerbs() {
        let html = `<h1 class="chapter new-section">🔁 4. Verbele lecției — Präsens · Präteritum · Perfekt · 🔧 Passiv</h1>
            <div class="andreea-note">
                <div class="andreea-note-content">
                    <div class="speaker">📌 Andreea îți spune:</div>
                    <div>Steaua lecției = Passivul (caseta cyan 🔧). Passiv = werden + Partizip II. Secretul e participiul: regulat (gemacht), tare (gestrichen), pe -ieren fără ge- (repariert), neseparabil fără ge- (überwiesen). La Passiv Perfekt e WORDEN, nu geworden! Toate verificate pe PONS.</div>
                </div>
            </div>`;
        if (typeof verbsData !== 'undefined') {
            verbsData.forEach((v, idx) => {
                ...
                html += `<div class="verb-card">
                    ...
                    ${v.passiv ? `<div class="passiv-box"><div class="de">🔧 Passiv: ${v.passiv}</div></div>` : ''}
